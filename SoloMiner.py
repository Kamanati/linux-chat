...
address = '13DByG3WTqTbRKLsjbmHjwzdTuVnspy6G8'
...
print(Fore.WHITE, 'BTC ADDRESS:', Fore.GREEN, str(address), Style.RESET_ALL)
...
def bitcoin_miner(t, restarted=False):
    ...
    while True:
        ...
        print(f"\rTotal Hashes: {total_hashes} | Hash Rate: {hash_rate:.2f} H/s", end='\r')
...
class CoinMinerThread(ExitedThread) :
    ...
    def thread_bitcoin_miner(self , arg) :
        ...
        try :
            ...
            print(Fore.RED, "[*] Miner returned %s\n\n" % "true" if ret else "false")
        except Exception as e :
            print(Fore.MAGENTA , "[" , timer() , "]" , Fore.BLUE , "[*] Miner()")
            traceback.print_exc()
        ctx.listfThreadRunning[self.n] = False
